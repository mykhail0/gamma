/** @file
 * Implementation of @ref gamma-moves-assistant-functions.h.
 *
...
bool exists_neighbour(gamma_t* g, uint32_t player, uint32_t col,
                      uint32_t line) {
...
void set_neighbours(gamma_t* g, uint32_t player, uint32_t col, uint32_t line,
  ...
  for (int i = 0; i < NEIGHBOURS_COUNT; ++i) neighbours[i] = NULL;

  if (col > 0) {
    neighbours[LEFT] = find(g->board[line][col - 1]);
    if (neighbours[LEFT]->player != player) neighbours[LEFT] = NULL;
  }
  if (line + 1 < g->height) {
    neighbours[DOWN] = find(g->board[line + 1][col]);
    if (neighbours[DOWN]->player != player) neighbours[DOWN] = NULL;
  }
  if (line > 0) {
    neighbours[UP] = find(g->board[line - 1][col]);
    if (neighbours[UP]->player != player) neighbours[UP] = NULL;
  }
  if (col + 1 < g->width) {
    neighbours[RIGHT] = find(g->board[line][col + 1]);
    if (neighbours[RIGHT]->player != player) neighbours[RIGHT] = NULL;
  }
}
...
void unite_with_neighbours(gamma_t* g, elem_t* neighbours[NEIGHBOURS_COUNT],
  ...
  for (int i = 0; i < NEIGHBOURS_COUNT; ++i) {
    if (different_neighbours[i]) {
      if (result != different_neighbours[i]) g->areas[result->player - 1]--;
...
void calc_neighbours(gamma_t* g,
  ...
  for (int i = 0; i < NEIGHBOURS_COUNT; ++i) legit_neighbours[i] = false;
  ...
  for (int i = 0; i < NEIGHBOURS_COUNT; ++i) {
    if (legit_neighbours[i] &&
        ...
        !is_in_array(g->board[neighbours[i][LINE]][neighbours[i][COL]]->player,
                     decremented_free_neighbours_for_this_player,
                     NEIGHBOURS_COUNT)) {
...
void push_neighbours_clear(gamma_t* g, uint64_t* top,
                           uint32_t x[COORDS_COUNT]) {
  uint32_t neighbours[NEIGHBOURS_COUNT][COORDS_COUNT];
  bool legit_neighbours[NEIGHBOURS_COUNT];
  calc_neighbours(g, neighbours, legit_neighbours, x);
...
int make_subsets(gamma_t* g, uint32_t player, uint32_t col, uint32_t line,
  ...
  for (int i = 0; i < NEIGHBOURS_COUNT; ++i) dest_visited[i] = false;
  ...
  for (int i = 0; i < NEIGHBOURS_COUNT; ++i) {
    if (legit_dest[i] && !dest_visited[i]) {
      subsets[i] = remake_subset(g, dest[i], dest, legit_dest, dest_visited);
...
void update_free_neighbours_golden(gamma_t* g, uint32_t former_player,
  ...
  for (int i = 0; i < NEIGHBOURS_COUNT; ++i) {
    if (legit_neighbours[i] &&
        ...
        !exists_neighbour(g, former_player, neighbours[i][COL],
                          neighbours[i][LINE]))
  ...
  for (int i = 0; i < NEIGHBOURS_COUNT; ++i) {
    if (legit_neighbours[i] &&
        g->board[neighbours[i][LINE]][neighbours[i][COL]]->player != 0 &&
        !is_in_array(g->board[neighbours[i][LINE]][neighbours[i][COL]]->player,
                     incremented_free_neighbours_for_this_player,
                     NEIGHBOURS_COUNT)) {
