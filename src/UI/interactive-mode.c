...
static void print_player_stats(gamma_t* g, uint32_t player) {
  ...
  if (g->areas[player - 1] == g->areas_number) printf("\033[m");

  if (g->golden_not_used[player - 1]) printf(" and did not use golden move");
}
...
static void print_board(gamma_t* g, uint32_t player) {
  ...
  for (uint32_t i = 0; i < g->height; ++i) {
    for (uint32_t j = 0; j < g->width; ++j) {
      ...
      if (g->board[i][j]->player == player) printf("\033[32m");  // na zielono

      printf("%s", buffer);

      if (g->height - i - 1 == cursorY && j == cursorX) printf("\033[m");
      if (g->board[i][j]->player == player) printf("\033[m");
    }
    printf("\n");
  }
 ...
 * @param[in] ch             - wskaźnik na kod znaku, który trzeba
 * zinterpretować,
 * @param[in] EOT_reached    - tu zapisywana informacja, czy ctrl+D zostało
 * napotkane,
 * @param[in] unfinishedANSI - tu zapisywana informacja, czy `ch` trzeba
 * aktualizować, po wywołaniu `make_move`.
    ...
    *ch = getchar();
    ...
    if (*ch == '[') {
      *ch = getchar();
      ...
      if (*ch == 'A') cursorUp();
      if (*ch == 'B') cursorDown();
      if (*ch == 'C') cursorForward();
      if (*ch == 'D') cursorBack();
      if (*ch < 'A' || 'D' < *ch) *unfinishedANSI = true;
    } else {
      *unfinishedANSI = true;
    }
  }
...
static void play(gamma_t* g) {
  ...
  while (!EOT_reached && !game_finished) {
    if (player_can_move) {
      ...
      player_can_move =
          make_move(g, &player, &c, &EOT_reached, &unfinishedANSI);
      if (errno == EINVAL) {
        errno = EINVAL;
        return;
      }
